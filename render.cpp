...
void draw_tree(tree::node_t *parent, node_t *gv_parent, graph_t *gv_graph) {
    ...
    if (parent->left_child && parent->left_child->right_sibling && parent->left_child->right_sibling->mark == 0) {
        ...
        agsafeset(gv_right_child, (char *) "label", (char *) label(parent->left_child->right_sibling).c_str(),
                  (char *) "");
    } else {
...
void print_tree_pre(tree::node_t *tree) {
    if (!tree || tree->mark != 0) return;
    std::cout << tree->value << " ";
    print_tree_pre(tree->left_child);
    if (tree->left_child) print_tree_pre(tree->left_child->right_sibling);
}

void print_tree_sym(tree::node_t *tree) {
    if (!tree || tree->mark != 0) return;
    print_tree_sym(tree->left_child);
    std::cout << tree->value << " ";
    if (tree->left_child) print_tree_sym(tree->left_child->right_sibling);
}
